...
def ensure_metadata_schema() -> None:
    ...
    with engine.begin() as connection:
        ...
        if "team" not in columns:
            connection.execute(text("ALTER TABLE users ADD COLUMN team VARCHAR(100)"))
        if "role" not in columns:
            connection.execute(text("ALTER TABLE users ADD COLUMN role VARCHAR(50) DEFAULT 'user'"))
        if "approval_status" not in columns:
            connection.execute(text("ALTER TABLE users ADD COLUMN approval_status VARCHAR(40) DEFAULT 'APPROVED'"))
        ...
        connection.execute(text("UPDATE users SET approval_status = COALESCE(approval_status, 'APPROVED')"))
        connection.execute(text("ALTER TABLE users ALTER COLUMN approval_status SET DEFAULT 'APPROVED'"))
        connection.execute(text("ALTER TABLE users ALTER COLUMN approval_status SET NOT NULL"))
